...
export async function searchReferral(req, res, next) {
    ...
    referralsService.searchReferral(req.body.text, id_progenitor, userid)
        .then(content => {
            res.send({ status: content.results.length > 0, content: content.results.length > 0 ? content.results : 'No user found', last_level: content.last_level, childs_count: content.childs_count })
        })
