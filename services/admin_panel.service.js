...
export async function approve_withdrawal(withdrawal_id) {
    ...
    await conexion.query('INSERT INTO notifications (owner, message, date) VALUES($1,$2,$3)', [withdrawal_request.id, `Your withdrawal has already been processed to the leals address you provided. Amount: ${withdrawal_request.amount / p2p_config.value_compared_usdt} leals`, new Date()])
...
export async function deny_withdrawal(withdrawal_id) {
    ...
    await conexion.query('UPDATE withdrawals SET status=($1) WHERE withdrawal_id=($2)', ['successful', withdrawal_id])
    await conexion.query('INSERT INTO history (owner, history_type, cash_flow, amount, widthdrawal_condition,currency) VALUES($1,$2,$3,$4,$5,$6)', [withdrawal_request.owner, 'withdrawal', 'outcome', withdrawal_request.amount, 'successful', 'usdt'])
    await conexion.query('INSERT INTO notifications (owner, message, date) VALUES($1,$2,$3)', [withdrawal_request.id, `We are very sorry your withdrawal was not processed`], new Date())
