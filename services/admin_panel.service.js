...
export async function list_users(condition) {
    ...
    switch (condition) {
        ...
        case 'deleted':
            ...
            break;
        case 'with buys':
            users = await (await conexion.query('SELECT * FROM tickets INNER JOIN usuarios ON tickets.owner=usuarios.id WHERE tickets.status=($1) AND tickets.type=($2) GROUP BY tickets.onwer', ['finished', 'buy'])).rows
            break;
        case 'with sells':
            users = await (await conexion.query('SELECT * FROM tickets INNER JOIN usuarios ON tickets.owner=usuarios.id WHERE tickets.status=($1) AND tickets.type=($2) GROUP BY tickets.onwer', ['finished', 'sell'])).rows
            break;
        case 'admins':
