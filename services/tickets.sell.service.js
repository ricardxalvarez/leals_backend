...
export async function createTicket(data, userid) {
    ...
    const tickets = await (await conexion.query('SELECT * FROM tickets WHERE owner<>($1) AND type=($2) ORDER BY created_at', [userid, 'buy'])).rows
    ...
    await conexion.query("INSERT INTO tickets (amount, remain, type, created_at, owner) VALUES ($1, $2, $3, $4, $5)", [packag.leals_quantity, packag.leals_quantity, 'sell', new Date(), userid])
    return { status: true, content: "Ticket created successfully" }
}

export async function listTickets() {
    const tickets = await (await conexion.query('SELECT * FROM tickets WHERE type=($1)', ['sell'])).rows
    return tickets
}
