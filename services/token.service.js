...
export async function createTokenPhoneVerification(userid) {
    const code = Math.floor(100000 + Math.random() * 900000)
    const token = await conexion.query('INSERT INTO phone_verification_tokens(owner, code) VALUES($1, $2) RETURNING *', [userid, code])
    return token
}
...
export async function deleteTokensPhoneVerification(userid) {
    const tokens = await conexion.query('DELETE FROM phone_verification_tokens WHERE owner=($1)', [userid])
    return tokens
}