...
export async function wallet_info(userid) {
    ...
    const not_available_leals_balance = fix_number(wallet.not_available / p2p_config.value_compared_usdt)
    wallet.balance = fix_number(wallet.balance) || "0.00"
    wallet.not_available = fix_number(wallet.not_available) || "0.00"

    return { ...wallet, leals_balance, not_available_leals_balance }
}