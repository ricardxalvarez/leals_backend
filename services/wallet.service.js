...
export async function wallet_info(userid) {
    ...
    if (!wallet) return { ...await (await conexion.query('INSERT INTO wallets (owner) VALUES ($1) RETURNING *', [userid])).rows[0], leals_balance: 0 }
    const leals_balance = fix_number(wallet.balance / p2p_config.value_compared_usdt) || 0
