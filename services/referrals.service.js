...
export async function referralChildren({ iduser, level }) {
  let users = await (await conexion.query("SELECT id, nombre_usuario, avatar, id_sponsor, avatar, country_code as codigo_pais FROM usuarios WHERE id_progenitor=($1) OR id=($1) ORDER BY id_sponsor NULLS FIRST", [iduser])).rows
  function Node(user) {
    this.user = user,
      this.children = [];
  }
