...
export async function searchReferral(text, iduser, id) {
  ...
  if (text) {
    ...
    let usersList = results.filter(object => object.user.nombre_usuario.toLowerCase().includes(text.toLowerCase())).sort((a, b) => a.user.nombre_usuario - b.user.nombre_usuario).filter(object => object.user.nombre_usuario !== text).splice(0, 0, matchingElement);
