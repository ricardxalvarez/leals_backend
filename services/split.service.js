...
export async function split_info(user_id) {
    ...
    const network_withdrawls = await (await conexion.query('SELECT amount FROM withdrawals WHERE status=($1)', ['successful'])).rows.map(object => object.amount).reduce((partialSum, a) => partialSum + a, 0) / p2p_config.value_compared_usdt
    ...
    const my_withdrawls = await (await conexion.query('SELECT amount FROM withdrawals WHERE owner=($1) AND status=($2)', [user_id, 'successful'])).rows.map(object => object.amount).reduce((partialSum, a) => partialSum + a, 0) / p2p_config.value_compared_usdt
