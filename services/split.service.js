...
export async function split_info(user_id) {
    ...
    const my_buys = await (await conexion.query('SELECT orders.amount, tickets.ticket_id, tickets.owner, tickets.amount AS amount_to_sum FROM orders INNER JOIN tickets ON tickets.ticket_id=orders.ticket_buyer_id WHERE orders.status=($1) AND tickets.owner=($2)', ['successfull', user_id])).rows.map(object => object.amount_to_sum).reduce((partialSum, a) => partialSum + a, 0) * 3 / p2p_config.value_compared_usdt
    const my_sells = await (await conexion.query('SELECT orders.amount, tickets.ticket_id, tickets.owner, tickets.amount AS amount_to_sum FROM orders INNER JOIN tickets ON tickets.ticket_id=orders.ticket_seller_id WHERE orders.status=($1) AND tickets.owner=($2)', ['successfull', user_id])).rows.reduce((partialSum, a) => partialSum + a, 0) / p2p_config.value_compared_usdt
