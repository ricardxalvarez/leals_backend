...
export const add_business = {
    body: joi.object().keys({
        type: joi.string().lowercase().required().valid('local', 'local professional', 'mobility', 'professional online', 'buy s/p'),
        ...
        business_name: joi.alternatives().conditional('type', { is: 'local', then: joi.string().required().trim() }),
        ...
        opening_time: joi.alternatives().conditional('type', { is: 'local', then: joi.string().required().regex(/^([1-9]|0[1-9]|1[0-2]):[0-5][0-9] ([AaPp][Mm])$/) }),
        closing_time: joi.alternatives().conditional('type', { is: 'local', then: joi.string().required().regex(/^([1-9]|0[1-9]|1[0-2]):[0-5][0-9] ([AaPp][Mm])$/) }),
        fee_per_hour: joi.alternatives().conditional('type', { is: 'local professional', then: joi.number().required() }),
        fee_per_kilometer: joi.alternatives().conditional('type', { is: 'mobility', then: joi.number().required() }),
        driver_name: joi.alternatives().conditional('type', { is: 'mobility', then: joi.string().trim().required() }),
