...
export const add_business = {
    body: joi.object().keys({
        ...
        business_name: joi.alternatives().conditional('type', { is: 'local', then: joi.string().required().trim() }).required(),
        ...
        opening_time: joi.alternatives().conditional('type', { is: 'local', then: joi.string().required().regex(/^([1-9]|0[1-9]|1[0-2]):[0-5][0-9] ([AaPp][Mm])$/) }).required(),
        closing_time: joi.alternatives().conditional('type', { is: 'local', then: joi.string().required().regex(/^([1-9]|0[1-9]|1[0-2]):[0-5][0-9] ([AaPp][Mm])$/) }).required(),
        fee_per_hour: joi.alternatives().conditional('type', { is: 'local professional', then: joi.number().required() }).required(),
        fee_per_kilometer: joi.alternatives().conditional('type', { is: 'mobility', then: joi.number().required() }).required(),
        driver_name: joi.alternatives().conditional('type', { is: 'mobility', then: joi.string().trim().required() }).required(),
        registration_number_car: joi.alternatives().conditional('type', { is: 'mobility', then: joi.string().trim().required() }).required(),
        product_name: joi.alternatives().conditional('type', { is: 'buy s/p', then: joi.string().trim().required() }).required(),
        product_price: joi.alternatives().conditional('type', { is: 'buy s/p', then: joi.string().trim().required() }).required(),
