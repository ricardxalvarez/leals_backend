    ...
    async function timeout() {
        ...
        for (let i = 0; i < wallets.length; i++) {
            ...
            if (seconds_remain <= 0) {
                ...
                const direct_businesses = await (await conexion.query('SELECT businesses.*, usuarios.id FROM businesses INNER JOIN usuarios ON usuarios.id=businesses.owner WHERE usuarios.id_sponsor=($1)', [wallet.owner])).rows
                let valid_new_businesses_count = 0
                for (let j = 0; j < direct_businesses.length; j++) {
                    const business = direct_businesses[j];
                    const seconds_remain = (((new Date().getTime() - new Date(business.created_at).getTime()) / 1000) - seconds_in_a_week) * -1
                    if (seconds_remain >= 0) valid_new_businesses_count++;
                }


                const user_percentage_by_ads = rule_ads.find(object => (object.ads_quantity >= valid_advertises_count) && object.new_businesses >= valid_new_businesses_count)?.percentage || rule_ads.find(object => object.ads_quantity === 0)?.percentage || 1
